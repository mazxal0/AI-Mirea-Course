...
def report(
    ...
    top_cats = top_categories(df, top_k=max_rows_k_categories, max_columns=top_k_categories)
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Есть ли колонки с одинаковым значением: **{quality_flags['has_constant_columns']}**\n")
        f.write(f"- Есть ли категориальные признаки с большим количеством уникальных значений: **{quality_flags['has_high_cardinality_categoricals']}**\n")
        f.write(f"- Имеют ли числовые признаки большую долю нулевых значений: **{quality_flags['has_many_zero_values']}**\n\n")

        f.write("## Новые пороги|настройки\n\n")
        f.write(f"Сколько категориальных признаков выводить: **{top_k_categories}**\n")
        f.write(f"Сколько top-значений выводить для категориальных признаков: **{max_rows_k_categories}**\n\n")
